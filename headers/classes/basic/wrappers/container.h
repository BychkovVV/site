  ...
  {template <typename TYPE> class Container : public BychkovVV::main::classes::basic::core::Standard
     {protected:
      ...
      public:
        ...
        bool operator && (const TYPE &otherElement) const
          {Standard<TYPE> result(this);            
           result = value && otherElement;
           return result;
          }
        bool operator && (const Standard<TYPE> &otherElement) const
          {return operator &&(otherElement.getValue());
          }
        friend bool operator && (bool theValue, const Standard<TYPE> &element)
          {return theValue && element.getValue();             
          }
        bool operator || (const TYPE &otherElement) const
          {return value || otherElement;
          }
        bool operator || (const Standard<TYPE> &otherElement) const
          {return operator ||(otherElement.getValue());
          }
        ...
        std::strong_ordering operator <=> (const TYPE &otherElement) const
          {return value <=> otherElement;             
          }
        std::strong_ordering operator <=> (const Standard<TYPE> &otherElement) const
          {return value <=> otherElement;             
          }
        bool operator >= (const TYPE &otherElement) const
          {return value >= otherElement;             
