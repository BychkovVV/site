  ...
  {protected:
   ...
   public:
     ...
     virtual unsigned long long toBinaryStream(BinaryStream &target) const
       {unsigned char* bytes;
        unsigned long long countInBits = this->dataToBinary(bytes);
        target->setBytes(bytes, BinaryStream::getBytesCountInABytes(countInBits));
        return countInBits;
       }
     virtual unsigned long long dataToBinary(unsigned char* &target) const
